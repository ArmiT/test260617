...
/**
*
* ќсновные функции гостевой книги
*
*/
/**
*
* ‘ункции дл€ работы с базой данных
*
*/

/** recource db_connect ( string host, string user, string passwd, string dbname )
* ѕодключение к —”Ѕƒ и открытие базы данных
*/
...
/** ¬ыполн€ет запрос к Ѕƒ (placeholder)
*
* @param текст запроса
* @param*
* @return resource id
...
/**
* ƒобавление записи в гостевую книгу
*/
function gb_add($name, $email, $message, &$error)
{
	// провер€ем правильность заполнени€ полей
	$error = '';
	if(empty($name))
		$error['name'] = 'Ёто об€зательное поле';
	if(empty($message))
		$error['message'] = 'Ёто об€зательное поле';
	if(!empty($email) && !strings_isemail($email))
		$error['email'] = 'Ёто не email';

	// если не было ошибок -- добавл€ем
	if(!$error)
	{
		// чистим данные
...
$link = mysqli_connect("localhost","root", "","test");
	...
	if($rows)
	{
		...
		// печатаем ссылки на страницы (номер текущей страницы не €вл€етс€ ссылкой)
		echo '<div class=c>';
		for($i = 1; $i <= $pages; $i++)
		{
			if($i != $current)
		...
		// если это не полследн€€ страница печатаем ссылку "ƒальше"
		if($current < $pages)
